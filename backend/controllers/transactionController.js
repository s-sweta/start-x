...
const simulateCustomerBehavior = (customer, products, activeStrategies, enabledGateways) => {
    const behavior = {
        willBuy: false,
        selectedProducts: [],
        paymentMethod: 'CARD',
    ...
    if (behavior.willBuy) {
        ...
        // Choose payment method based on persona and available gateways
        const availableGateways = enabledGateways.filter(gateway => gateway.isEnabled);
        if (availableGateways.length === 0) {
            // Fallback to CARD if no gateways are configured
            behavior.paymentMethod = 'CARD';
        } else {
            const gatewayTypes = availableGateways.map(g => g.type);
            
            if (customer.persona === 'MOBILE_FIRST') {
                // Prefer UPI and WALLET for mobile-first customers
                const mobilePreferred = gatewayTypes.filter(type => ['UPI', 'WALLET'].includes(type));
                if (mobilePreferred.length > 0) {
                    behavior.paymentMethod = mobilePreferred[Math.floor(Math.random() * mobilePreferred.length)];
                } else {
                    behavior.paymentMethod = gatewayTypes[Math.floor(Math.random() * gatewayTypes.length)];
                }
            } else if (customer.persona === 'IMPULSE_BUYER') {
                // Prefer CARD and UPI for impulse buyers
                const impulsePreferred = gatewayTypes.filter(type => ['CARD', 'UPI'].includes(type));
                if (impulsePreferred.length > 0) {
                    behavior.paymentMethod = impulsePreferred[Math.floor(Math.random() * impulsePreferred.length)];
                } else {
                    behavior.paymentMethod = gatewayTypes[Math.floor(Math.random() * gatewayTypes.length)];
                }
            } else {
                // Random selection for other personas
                behavior.paymentMethod = gatewayTypes[Math.floor(Math.random() * gatewayTypes.length)];
            }
        }
    }
...
exports.simulateTransactions = async (req, res) => {
    try {
        ...
        const [customers, products, strategies, paymentGateways] = await Promise.all([
            Customer.find({ store: store._id, isActive: true }),
            Product.find({ store: store._id }),
            Strategy.find({ store: store._id, isActive: true }),
            PaymentGateway.find({ store: store._id, isEnabled: true })
        ]);
        ...
        console.log('🔍 DEBUG - Found payment gateways:', paymentGateways.length);
        ...
        for (let day = 0; day < days; day++) {
            ...
            for (const customer of customers) {
                ...
                if (Math.random() < dailyVisitChance) {
                    ...
                    const behavior = simulateCustomerBehavior(customer, products, strategies, paymentGateways);
                    ...
                    if (behavior.willBuy && behavior.selectedProducts.length > 0) {
                        ...
                        // Simulate payment success/failure based on gateway configuration
                        const selectedGateway = paymentGateways.find(g => g.type === behavior.paymentMethod);
                        const successRate = selectedGateway ? selectedGateway.successRate / 100 : 0.95; // Default to 95% if gateway not found
                        const paymentStatus = Math.random() < successRate ? 'SUCCESS' : 'FAILED';
